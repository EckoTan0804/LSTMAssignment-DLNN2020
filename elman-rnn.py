...
def backward(activations, clipping=True, scale=True):
    ...
    for t in reversed(range(inputs.shape[1])):
        ...
        # first, we need to compute the gradients of the variable
        # closest to the loss function, which is the softmax output p
        # but here I skip it directly to the gradients of the unnormalized scores o
        # because basically dL / do = p - y
        # from the cross entropy gradients.
        # (the explanation is a bit too long to write here)
        do = ps[t] - ys[t]
